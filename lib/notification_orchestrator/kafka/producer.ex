defmodule NotificationOrchestrator.Kafka.Producer do
  @moduledoc """
  Kafka producer for publishing notification events and delivery status.

  ## Design Patterns Used:
  - **Circuit Breaker**: Graceful degradation when Kafka is unavailable
  - **Retry Pattern**: Exponential backoff for failed publishes
  - **Async Pattern**: Non-blocking event publishing

  ## Topics Published:
  - notification-status: Notification delivery status updates
  - notification-analytics: Analytics events for notification tracking
  """
  use GenServer
  require Logger

  @status_topic "notification-status"
  @analytics_topic "notification-analytics"
  ...
  @doc "Publish notification delivery status"
  def publish_status(notification_id, status, metadata \\ %{}) do
    event = %{
    ...
    GenServer.call(__MODULE__, {:publish, @status_topic, event})
  catch
    :exit, _ -> {:error, :producer_unavailable}
  end

  @doc "Publish notification analytics event"
  def publish_analytics(event_type, data) do
    event = %{
      event_type: event_type,
      data: data,
      timestamp: DateTime.utc_now() |> DateTime.to_iso8601()
    }
    GenServer.cast(__MODULE__, {:publish_async, @analytics_topic, event})
  end
  ...
  def init(_opts) do
    ...
    if enabled do
      ...
      Logger.info("[NotificationKafkaProducer] Kafka disabled - events will not be published")
    end
  ...
  def handle_info(:connect, state) do
    case start_brod_client(state) do
      {:ok, new_state} ->
        Logger.info("[NotificationKafkaProducer] Successfully connected to Kafka")
        {:noreply, %{new_state | circuit_state: :closed, failure_count: 0}}

      {:error, reason} ->
        Logger.warning("[NotificationKafkaProducer] Failed to connect: #{inspect(reason)}")
        schedule_reconnect()
  ...
  defp start_brod_client(state) do
    ...
    client_id = :notification_kafka_producer
  ...
  defp do_publish(topic, event, state) do
    ...
    case :brod.produce_sync(state.client_id, topic, partition_fun(partition_key), partition_key, message) do
      :ok ->
        :telemetry.execute(
          [:notification_orchestrator, :kafka, :event_published],
          %{count: 1},
          %{topic: topic}
        )
        :ok
      ...
      {:error, reason} ->
        Logger.warning("[NotificationKafkaProducer] Failed to publish to #{topic}: #{inspect(reason)}")
        {:error, reason}
    end
  rescue
    e ->
      Logger.error("[NotificationKafkaProducer] Exception publishing: #{inspect(e)}")
      {:error, :exception}
  end

  defp extract_partition_key(event) do
    event[:notification_id] || event["notification_id"] || event[:user_id] || event["user_id"] || "default"
  end
  ...
  defp handle_publish_failure(state, _reason) do
    ...
    if new_failure_count >= @max_failures do
      Logger.error("[NotificationKafkaProducer] Circuit breaker OPEN")
      schedule_reconnect()
