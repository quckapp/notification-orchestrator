defmodule NotificationOrchestrator.CircuitBreaker do
  @moduledoc """
  Circuit breaker wrapper using the fuse library.

  Provides fault tolerance for external service calls (Firebase, APNs, Email, Kafka).
  ...
  ## Critical Protection:
  Push notification providers (Firebase, APNs) are wrapped with circuit breakers
  to prevent cascade failures when provider APIs are unavailable.

  ## Usage:

      CircuitBreaker.call(:firebase, fn ->
        Firebase.send_notification(device_token, payload)
      end)

      CircuitBreaker.call(:apns, fn ->
        APNs.send_notification(device_token, payload)
      end, default: {:error, :apns_unavailable})
  """

  require Logger

  @fuses %{
    firebase: :notification_firebase_fuse,
    apns: :notification_apns_fuse,
    email: :notification_email_fuse,
    kafka: :notification_kafka_fuse,
    redis: :notification_redis_fuse,
    http: :notification_http_fuse
  }
  ...
  defp fuse_options(service) do
    config = Application.get_env(:notification_orchestrator, :circuit_breaker, %{})
    service_config = Map.get(config, service, %{})

    # Push notification providers get stricter settings
    defaults = case service do
      :firebase -> %{max_failures: 3, time_window: 10_000, reset_timeout: 60_000}
      :apns -> %{max_failures: 3, time_window: 10_000, reset_timeout: 60_000}
  ...
  defp emit_circuit_open_telemetry(service) do
    :telemetry.execute(
      [:notification_orchestrator, :circuit_breaker, :open],
      %{count: 1},
      %{service: service}
    )
  end
